...
class ColorGridPlotter:
    """Creates 2D color grid plots for map x ability configuration analysis."""
    ...
    def create_color_grid_figure(
        self,
        data: pd.DataFrame,
        comparison_type: str = 'global',
        output_path: str = None,
        interpolation: str = 'nearest',
    ) -> plt.Figure:
        """Create a 2D color grid figure showing map x ability analysis.
        
        Args:
            data: DataFrame with all experimental data
            comparison_type: Type of comparison ('global', 'row', 'column')
            output_path: Path to save the figure
            interpolation: Matplotlib interpolation mode for the heatmaps
        ...
        fig.suptitle(f'Map x Ability Configuration Analysis\n{subtitle}', 
                    fontsize=16, fontweight='bold')
        
        # Create 2D grids for plotting
        perf_grid = self._create_2d_grid(perf_diffs)
        gap_grid = self._create_2d_grid(gap_diffs)
        ...
        # Plot deliver/cut gap differences
        self._plot_2d_grid(
            ax2,
            gap_grid,
    ...
    def _plot_2d_grid(
        ...
        if is_gap or is_action_diff:
            # Orange-white-green: negative = smaller gap/differentiated, positive = larger gap/differentiated
            cmap = plt.cm.RdYlGn
        else:
        ...
        # Plot the grid
        im = ax.imshow(grid, cmap=cmap, aspect='auto', vmin=vmin, vmax=vmax, interpolation=interpolation)
...
def main():
    ...
    try:
        ...
        print("=" * 60)
        print("FULL 2D GRID ANALYSIS - MAPS x ABILITIES")
        print("=" * 60)
        ...
        if isinstance(df_or_dict, dict):
            ...
            for combination_key in sorted(df_or_dict.keys()):
                ...
                print(f"\n=== Creating 2D Grid Analysis (Maps x Abilities) ===\n")
                ...
                filename_parts = []
                filename_parts.append('full_grid')
                ...
                # Generate the comparison plots in blurred and softer modes
                comparison_types = [
                ...
                for comparison_type, type_name in comparison_types:
                    ...
                    # Create filenames for this comparison type
                    blurred_filename = f"blurred_{base_filename}_{comparison_type}.png"
                    softer_filename = f"softer_{base_filename}_{comparison_type}.png"
                    blurred_path = subfolder / blurred_filename
                    softer_path = subfolder / softer_filename

                    # Generate the figures
                    fig = plotter.create_color_grid_figure(
                        prepared_data,
                        comparison_type,
                        str(blurred_path),
                        interpolation='bilinear',
                    )
                    output_paths.append(blurred_path)
                    ...
                    output_paths.append(softer_path)
                    plt.close(fig)  # Close the figure to free memory
            ...
            print("\nDeliver/cut gap differences (vs baseline map + X=1.0):")
            for map_short in [plotter._extract_map_short_name(name) for name in analyzer.map_names]:
                if map_short in gap_diffs:
                    print(f"\n{map_short}:")
                    for ability_config in sorted(gap_diffs[map_short].keys(), key=ability_sort_key):
                        diff = gap_diffs[map_short][ability_config]
                        print(f"  {plotter._ability_label(ability_config)}: {diff:+.2f} deliver/cut gap")
