...
for seed in "${seeds[@]}"; do
    ...
    nohup python /home/samuel_lozano/cooked/training-DTDE-spoiled_broth.py $cluster $input_path $map_nr $lr $game_version $num_agents $num_epochs $seed $checkpoint_paths $rewards_only_on_delivery $random_initial_state $synergy_scaling_factor $specialization_penalty_scale $agent_to_train > $output_file 2>&1 &
    
    echo "Lanzado entrenamiento con seed=$seed -> argumentos: $cluster $input_path $map_nr $lr $game_version $num_agents $num_epochs $seed $checkpoint_paths $rewards_only_on_delivery $random_initial_state $synergy_scaling_factor $specialization_penalty_scale $agent_to_train"
