#!/usr/bin/env python3
"""
Ability Configuration Analysis Figure Generator

This script creates color grid plots comparing different agent ability configurations
across performance metrics. The visualization shows how different X values perform
relative to the X=1.0 baseline configuration.

Agent ability configurations tested:
- X=1.0: Agent 1 (1.0, 1.0), Agent 2 (1.0, 1.0) - baseline
...
Color coding:
- White: X=1.0 (baseline, difference=0)
...
class CooperativeAnalyzer:
    ...
    def __init__(self, study_name: Optional[str] = None, 
        ...
        # Define ability configurations for grid analysis
        # X values: 1.0, 0.8, 0.6, 0.4, 0.2
        # Agent 1: (1.0, X), Agent 2: (X, 1.0)
        self.x_values = [1.0, 0.8, 0.6, 0.4, 0.2]
        
        # Define experimental conditions mapping for ability configurations
        self.base_condition_mapping = {}
        for x_val in self.x_values:
            # For each map, create condition mapping for the specified game type only
            for map_name in [map_name_1, map_name_2]:
                ...
                condition_name = f'{map_short}_x{x_val:.1f}{collision_suffix}'
        ...
        # Define baseline condition (X=1.0) for comparison
        self.baseline_x = 1.0
...
class ColorGridPlotter:
    """Creates color grid plots for ability configuration analysis."""
    
    def __init__(self, x_values: List[float], game_type: str = 'classic'):
    ...
    def calculate_performance_differences(self, data: pd.DataFrame) -> Dict[str, Dict[float, float]]:
        """Calculate performance differences relative to baseline (X=1.0).
        
        Returns:
            Dictionary with structure: {condition_base: {x_value: difference}}
            where condition_base is like 'baseline' or 'encouraged' (without x_value and collision suffix)
        """
        performance_diffs = {}
        
        # Group by map only (since we're using a single game type)
        for map_name in data['map_name'].unique():
            map_short = data[data['map_name'] == map_name]['condition'].iloc[0].split('_')[0]
            
            collision_suffix = '_collision' if 'collision' in self.game_type else ''
            condition_base = f"{map_short}{collision_suffix}"
            performance_diffs[condition_base] = {}
            
            # Find baseline performance (X=1.0)
            baseline_condition = f"{map_short}_x1.0{collision_suffix}"
            baseline_data = data[data['condition'] == baseline_condition]
                
            if len(baseline_data) == 0:
                print(f"    Warning: No baseline data found for {condition_base}")
                continue
            
            baseline_deliveries = baseline_data['total_deliveries'].mean()
            
            # Calculate differences for each X value
            for x_val in self.x_values:
                condition_name = f"{map_short}_x{x_val:.1f}{collision_suffix}"
                condition_data = data[data['condition'] == condition_name]
                
                if len(condition_data) == 0:
                    performance_diffs[condition_base][x_val] = 0.0
                    continue
                ...
                if x_val == 1.0:
                    ...
                    performance_diffs[condition_base][x_val] = current_deliveries - baseline_deliveries
        
        return performance_diffs
    
    def calculate_specialization_differences(self, data: pd.DataFrame) -> Dict[str, Dict[float, float]]:
        """Calculate specialization coefficient differences relative to baseline (X=1.0).
        ...
        Returns:
            Dictionary with structure: {condition_base: {x_value: specialization_difference}}
        """
        ...
        for map_name in data['map_name'].unique():
            ...
            collision_suffix = '_collision' if 'collision' in self.game_type else ''
            condition_base = f"{map_short}{collision_suffix}"
            specialization_diffs[condition_base] = {}
            
            # Find baseline specialization (X=1.0)
            baseline_condition = f"{map_short}_x1.0{collision_suffix}"
            baseline_data = data[data['condition'] == baseline_condition]
                
            if len(baseline_data) == 0:
                print(f"    Warning: No baseline data found for specialization {condition_base}")
                continue
            ...
            for x_val in self.x_values:
                ...
                if len(condition_data) == 0:
                    specialization_diffs[condition_base][x_val] = 0.0
                    continue
                ...
                if x_val == 1.0:
                    ...
                    # Difference from baseline
                    specialization_diffs[condition_base][x_val] = current_spec - baseline_spec
    ...
    def create_color_grid_figure(self, data: pd.DataFrame, output_path: str = None) -> plt.Figure:
        """Create color grid plot showing ability configuration performance and specialization.
        ...
        # Create figure with subplots: 2 rows per condition (performance + specialization)
        n_conditions = len(performance_diffs)
        fig, axes = plt.subplots(n_conditions * 2, 1, figsize=(12, 4 * n_conditions))
        
        # Ensure axes is always a list
        if n_conditions == 1:
            axes = [axes] if n_conditions * 2 == 1 else axes
        
        fig.suptitle('Ability Configuration Analysis\n(Relative to X=1.0 Baseline)', 
                    fontsize=16, fontweight='bold')
        
        # Find the maximum absolute differences for color scaling
        # Performance differences
        all_perf_diffs = []
        for condition_diffs in performance_diffs.values():
            all_perf_diffs.extend(condition_diffs.values())
        ...
        all_spec_diffs = []
        for condition_diffs in specialization_diffs.values():
            all_spec_diffs.extend(condition_diffs.values())
        ...
        # Create color grids for each condition
        condition_names = list(performance_diffs.keys())
        
        for i, condition_name in enumerate(condition_names):
            # Performance subplot (top row for each condition)
            perf_ax = axes[i * 2]
            self._create_single_color_grid(perf_ax, condition_name, performance_diffs[condition_name], 
                                         max_abs_perf_diff, 'Performance Difference (Deliveries)', 
                                         f'{condition_name} - Performance')
            
            # Specialization subplot (bottom row for each condition)
            spec_ax = axes[i * 2 + 1]
            spec_diffs = specialization_diffs.get(condition_name, {})
            self._create_single_color_grid(spec_ax, condition_name, spec_diffs, 
                                         max_abs_spec_diff, 'Specialization Difference', 
                                         f'{condition_name} - Specialization')
        
        # Add colorbars
        # Performance colorbar (left side)
        perf_cbar_ax = fig.add_axes([0.05, 0.15, 0.02, 0.7])  # [left, bottom, width, height]
        
        # Create a dummy image for the performance colorbar
        ...
        perf_cbar = fig.colorbar(mappable, cax=perf_cbar_ax)
        perf_cbar.set_label('Performance Difference\n(Deliveries)', fontsize=10, fontweight='bold')
        ...
        # Specialization colorbar (right side)
        spec_cbar_ax = fig.add_axes([0.92, 0.15, 0.02, 0.7])  # [left, bottom, width, height]
        spec_cbar = fig.colorbar(mappable, cax=spec_cbar_ax)
        spec_cbar.set_label('Specialization Difference\n(Index)', fontsize=10, fontweight='bold')
        ...
        fig.text(0.5, 0.02, 
                'Blue: Better/Higher than X=1.0 baseline | Red: Worse/Lower than X=1.0 baseline | White: X=1.0 baseline (reference)',
                ha='center', fontsize=10, style='italic')
        
        # Adjust layout
        plt.tight_layout()
        plt.subplots_adjust(bottom=0.1, left=0.12, right=0.88)
    ...
    def _create_single_color_grid(self, ax, condition_name: str, condition_diffs: Dict[float, float], 
                                max_abs_diff: float, metric_label: str, title: str):
        """Create a single color grid subplot.
        
        Args:
            ax: Matplotlib axis to plot on
            condition_name: Name of the condition
            condition_diffs: Dictionary of X values to difference values
        ...
        grid_data = np.zeros((1, len(self.x_values)))  # Single row, X columns
        colors = np.zeros((1, len(self.x_values)))
        
        for j, x_val in enumerate(self.x_values):
            diff = condition_diffs.get(x_val, 0.0)
            grid_data[0, j] = diff
            ...
            if x_val == 1.0:
                ...
                if is_specialization:
                    ...
                    colors[0, j] = diff / max_abs_diff if max_abs_diff > 0 else 0.0
                else:
        ...
        # Plot the grid
        im = ax.imshow(colors.reshape(1, -1), cmap=cmap, vmin=vmin, vmax=vmax, 
                      aspect='auto', interpolation='nearest')
        
        # Set labels
        ax.set_title(title, fontsize=12, fontweight='bold')
        ax.set_xticks(range(len(self.x_values)))
        ax.set_xticklabels([f'X={x:.1f}' for x in self.x_values], fontsize=10)
        ax.set_yticks([])
        ...
        for j, x_val in enumerate(self.x_values):
            ...
            if is_specialization:
                ...
                normalized_value = abs(colors[0, j])
                ...
                text_color = 'black' if abs(colors[0, j]) < 0.5 else 'white'
                text_val = f'{diff:.1f}'   # Fewer decimal places for deliveries
            
            ax.text(j, 0, text_val, ha='center', va='center', 
...
def setup_argument_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help='Directory to save output figures (default: {cluster_path}/data/samuel_lozano/cooked/grid_ability_analysis_figures)'
    )
...
def main():
    ...
    try:
        ...
        if args.output_dir is None:
            output_dir_base = f"{local_path}/data/samuel_lozano/cooked/grid_ability_analysis_figures"
        else:
        ...
        print("=" * 60)
        print("ABILITY CONFIGURATION ANALYSIS - COLOR GRID PLOTS")
        print("=" * 60)
        ...
        if isinstance(df_or_dict, dict):
            ...
            for combination_key in sorted(df_or_dict.keys()):
                ...
                filename_parts = ["grid_ability_analysis"]
                ...
                plotter = ColorGridPlotter(analyzer.x_values, analyzer.game_type)
